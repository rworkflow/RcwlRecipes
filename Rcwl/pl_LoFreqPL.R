## LoFreq PL
#' @include pl_lofreq_realign.R tl_LoFreq.R
...
s1 <- cwlStep(id = "tbamR", run = lofreq_realign,
              In = list(ref = "ref",
                        bam = "tbam"))
s2 <- cwlStep(id = "nbamR", run = lofreq_realign,
              In = list(ref = "ref",
                        bam = "nbam"))
...
LoFreqPL <- cwlWorkflow(requirements = list(requireSubworkflow()),
                        inputs = InputParamList(p1, p2, p3, p4, p5, p6, p7),
                        outputs = OutputParamList(o1, o2, o3, o4))
LoFreqPL <- LoFreqPL + s1 + s2 + s3
